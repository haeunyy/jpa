...
public class FieldAccessTests {
	 ...
	 *  만약 @Access를 설정하지 않으면 @Id의 위치를 기준으로 접근방식이 설정된다. */
	@Test
	public void field_접근_테스트() {
		...
		/* 기본적으로 JPA에서는 필드 접근을 사용한다는 것을 확인할 수 있다. 
		 * (작성한 메소드가 호출되지 않았기 때문에) getter 접근 없이 사용된다. 
		 * 따라서 @Access(AccessType.FIELD)속성은 생략 가능하다. 
		 * 하지만 다른 로직을 처리하거나 값을 검증하는 추가 로직을 수행해야하는 경우에는 
		 * 프로퍼티 접근 방식을 혼용해서 사용하기도 한다. */
	}
