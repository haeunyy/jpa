...
public class EmbeddedkeyTests {
	 ...
	 * 1. @Embeddable : @Embeddable클래스에 복합 키를 정의하고 엔티티에 @EmbeddedId를 이용해 복합 키 클래스를 매핑한다. 
	 * 
	 * 2. @IdClass : 복합키를 필드로 정의한 클래스를 이용해 엔티티 클래스에 해당하는 필드에 @Id를 매핑한다. 
	 * 
	 * 두방식 모두 복합키 클래스는 영속성 컨텍스트가 관리하지 않는다는 특징이 있으며 , 큰 기능적 차이도 존재하지 않는다. 
	 * 다만 @Embeddable이 조금 더 객체 지향적이고 @IdClass 는 관계형 데이터 베이스에 가까운 방법 */
	...
	public void 임베디드_아이디_사용한_복합키_테이블_매핑_테스트() {
		...
		String nativeSQL = 
				"SELECT UC.CONSTRAINT_NAME, UC.CONSTRAINT_TYPE, UCC.COLUMN_NAME " +
				  ...
				  "WHERE UCC.TABLE_NAME = 'TBL_MEMBER_SECTION06_SUB01'" +
